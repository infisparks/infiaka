    ...
    <section className="bg-white border border-[#E2E8F0] rounded-xl p-5 space-y-4 shadow-sm w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1">
          <label className="text-xs font-bold text-[#718096] uppercase select-none">Notes for Patient (Treatment/Surgical/Others)</label>
          <textarea
            rows={3}
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold text-[#718096] uppercase select-none">Private Notes (These will not be printed)</label>
          <textarea
            rows={3}
            value={privateNotes}
            onChange={(e) => setPrivateNotes(e.target.value)}
            placeholder="Clinical reminders, diagnostic notes..."
            className="w-full p-2 border border-[#CBD5E0] rounded-md text-sm focus:outline-none bg-slate-50/50"
          />
        </div>
      </div>
    </section>
