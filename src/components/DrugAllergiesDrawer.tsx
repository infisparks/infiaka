...
const DEFAULT_NOTES = ["Severe allergy", "Mild rash", "Anaphylaxis risk", "Avoid completely"];
...
interface DrugAllergiesDrawerProps {
  isOpen: boolean;
  onClose: () => void;
...
/* ═══════════════════════════════════════════════════════════════════
   DrugAllergiesDrawer Component
═══════════════════════════════════════════════════════════════════ */
export default function DrugAllergiesDrawer({
  isOpen,
  onClose,
  allergies,
  setAllergies,
}: DrugAllergiesDrawerProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [names, statuses, notes] = await Promise.all([
        fetchOptions(120, DEFAULT_NAMES),
        fetchOptions(121, DEFAULT_STATUSES),
        fetchOptions(122, DEFAULT_NOTES)
      ]);
      if (active) {
  ...
  const addAllergy = (name: string) => {
    ...
    setAllergies((p) => [...p, newAllergy]);
    incrementOption(120, name.trim());
    setSearchVal("");
    setSearchOpen(false);
    setSearchHi(-1);
  };

  const patch  = (id: string, diff: Partial<DrugAllergy>) => setAllergies((p) => p.map((c) => (c.id === id ? { ...c, ...diff } : c)));
  const remove = (id: string) => setAllergies((p) => p.filter((c) => c.id !== id));
  ...
  const onDragOver  = (e: React.DragEvent, i: number) => {
    ...
    const r = [...allergies]; const [m] = r.splice(dragIdx.current, 1); r.splice(i, 0, m);
    dragIdx.current = i; setAllergies(r);
  };
  ...
  const handleSearchKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setSearchHi((p) => Math.min(p + 1, list.length - 1)); }
      ...
      e.preventDefault();
      if (searchHi >= 0 && list[searchHi]) addAllergy(list[searchHi]);
      else addAllergy(searchVal);
    }
      ...
      <div className="w-[88vw] max-w-[850px] h-full bg-white shadow-2xl flex flex-col overflow-hidden animate-slide-in relative border-l border-[#E2E8F0]">
        ...
        <div className="px-5 py-4 border-b border-[#F1F5F9] flex items-center justify-between shrink-0 bg-[#FAFBFC]">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[13px] font-extrabold text-[#1E293B]">Drug Allergy</span>
          </div>
        ...
        <div className="p-5 flex-1 flex flex-col space-y-4 overflow-y-auto min-h-0">
          ...
          <div className="relative">
            <div className="relative flex items-center">
              ...
              />
              {searchVal.trim() && (
                <button type="button" onClick={() => addAllergy(searchVal)}
            ...
            {searchOpen && (() => {
              ...
              return (
                <div className="absolute left-0 right-0 top-full mt-1.5 z-[60] bg-white border border-[#E2E8F0] rounded-xl shadow-xl overflow-hidden max-h-52 overflow-y-auto">
                  {list.map((opt, i) => (
                    <div key={opt} onMouseDown={() => addAllergy(opt)}
          ...
          <div className="flex-1 border border-[#E2E8F0] rounded-xl overflow-y-auto min-h-0 bg-[#FAFBFC]">
            ...
            {allergies.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-[#CBD5E0]">
                ...
                <span className="text-[11px] font-bold uppercase tracking-wider">No drug allergies added yet</span>
              </div>
            ) : (
                    ...
                    <div className="flex items-stretch min-h-[38px] w-full">
                      ...
                      <div className="relative w-[28%] shrink-0 border-r border-[#E2E8F0] flex items-center bg-white">
                        <input type="text" value={item.name}
                          ...
                          onBlur={() => handleInputBlur(120, item.name)}
                      ...
                      <div className="relative w-[22%] shrink-0 border-r border-[#E2E8F0] flex items-center bg-white">
                        <input type="text" value={item.status}
                          ...
                          onBlur={() => handleInputBlur(121, item.status)}
                      ...
                      <div className="relative w-[22%] shrink-0 border-r border-[#E2E8F0] flex items-center bg-white">
                        <input type="text" value={item.notes}
                          ...
                          onBlur={() => handleInputBlur(122, item.notes)}
