...
export default function IpdPage() {
  ...
  // Delete modal state
  const [recordToDelete, setRecordToDelete] = useState<IpdRecord | null>(null);
  ...
  const confirmDeleteRecord = async () => {
    ...
    setIsDeleting(true);
    try {
      const { error } = await supabase.from("aka_ipd").delete().eq("id", recordToDelete.id);
      if (error) throw error;
                ...
                <tbody className="divide-y divide-[#E5E7EB] text-[#111827]">
                  {filteredRecords.length === 0 ? (
                    ...
                    filteredRecords.map((r, idx) => (
                      <tr
                        key={r.id}
                        className={`transition-colors border-b border-[#E5E7EB] hover:bg-purple-50/60 ${
                          idx % 2 === 0 ? "bg-white" : "bg-[#F8FAFC]"
                        }`}
                      >
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] whitespace-nowrap text-slate-600 font-medium">{r.surgery_date || "-"}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] whitespace-nowrap font-medium">{r.month || "-"}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] whitespace-nowrap font-bold text-slate-900">{r.patients_name}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] whitespace-nowrap">
                          <span className="px-2 py-0.5 bg-slate-100 border border-slate-200 rounded text-[11px] font-bold text-slate-700">
                            {r.hospital || "-"}
                          </span>
                        </td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] whitespace-nowrap text-center font-bold">{r.sex || "-"}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] text-slate-700 max-w-[200px] truncate font-medium" title={r.diagnosis}>{r.diagnosis || "-"}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] font-medium text-slate-800 max-w-[200px] truncate" title={r.surgery}>{r.surgery || "-"}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] whitespace-nowrap text-slate-700 font-medium">
                        ...
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] text-right font-bold text-slate-900">{fmtCurr(r.bill_amount)}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] text-right font-semibold text-emerald-700">{fmtCurr(r.net_pkg)}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] text-right font-semibold text-indigo-700">{fmtCurr(r.surgeons_fees)}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] text-right text-slate-600 font-medium">{fmtCurr(r.hosp_charges)}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] text-right text-slate-600 font-medium">{fmtCurr(r.eqpmt_charges)}</td>
                        <td className="py-2.5 px-3 border-r border-[#E5E7EB] text-center">
                        ...
                        <td className="py-2.5 px-3 text-center whitespace-nowrap">
                          <div className="flex items-center justify-center gap-1.5">
                            <button
                              ...
                              className="p-1 hover:bg-indigo-50 rounded-md text-indigo-650 transition-colors cursor-pointer"
                              title="Edit Record"
                            >
                            ...
                            <button
                              ...
                              className="p-1 hover:bg-red-50 rounded-md text-red-600 transition-colors cursor-pointer"
                              title="Delete Record"
                            >
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-in fade-in duration-150 select-none">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-4 border border-slate-200 animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-extrabold text-slate-900">Delete IPD Record?</h3>
                <p className="text-xs text-slate-500 font-medium mt-0.5">
                  Are you sure you want to delete <span className="font-bold text-slate-800">{recordToDelete.patients_name}</span>? This action cannot be undone.
                </p>
              </div>
            </div>

            <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 text-xs space-y-1 text-slate-700">
              <div><span className="font-bold">Hospital:</span> {recordToDelete.hospital || "-"}</div>
              <div><span className="font-bold">Surgery:</span> {recordToDelete.surgery || "-"}</div>
              <div><span className="font-bold">Bill Amount:</span> {fmtCurr(recordToDelete.bill_amount)}</div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                disabled={isDeleting}
                className="px-5 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl shadow-md transition-all cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
              >
